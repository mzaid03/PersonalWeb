...
function App() {
  ...
  // Gentle floating for badge (respects reduced motion)
  useEffect(() => {
  ...
  // Smooth, GPU-friendly page transition (no blur)
  ...
  // Auto performance mode on lower-power devices
  useEffect(() => {
    const lowCore = typeof navigator !== 'undefined' && navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4;
    const poorMem = typeof performance !== 'undefined' && performance.memory && performance.memory.jsHeapSizeLimit < 1024 * 1024 * 1024 * 2; // <2GB
    if (lowCore || poorMem) {
                ...
                <div className="panel-grid about-grid">
                  ...
                  >
                    ...
                    <motion.img
                      ...
                      whileHover={{ scale: 1.03 }}
                    />
                    <motion.div className="badge-floating" animate={controls} aria-hidden>
                      🎓
                    </motion.div>
                  </motion.div>
